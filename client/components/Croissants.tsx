...
export interface CroissantData {
  nodes: {
    Croissant_HIGH: { geometry: THREE.BufferGeometry }
    Croissant_LOW: { geometry: THREE.BufferGeometry }
  }
...
function Croissant({ index, z, speed }: Positioning) {
  ...
  return (
    <Detailed ref={ref} distances={[20, 65, 80]}>
      <mesh
        geometry={nodes.Croissant_HIGH.geometry}
        material={materials.Croissant_LOW}
      />
      <mesh
        geometry={nodes.Croissant_LOW.geometry}
        material={materials.Croissant_LOW}
      />
    </Detailed>
  )
...
export default function Croissants({
  ...
  return (
    <Canvas
      gl={{ antialias: false }}
      dpr={[1, 1.5]}
      camera={{ position: [0, 0, 10], fov: 25, near: 0.01, far: depth + 15 }}
    >
