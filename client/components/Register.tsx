...
export default function Register() {
  ...
  const handleAdd = async (userData: UserData) => {
    ...
    try{
      ...
      setAlertData((prev) => ({...prev, messageBody:"Now get out there and drink some wine", text:"Profile Updated!", colour:"green" }))

      setAlert(() => true)
    }
    catch (error){
      setAlertData((prev) => ({...prev, messageBody:"Notify Jean Pierre!", text:"Something Went Wrong!", colour:"red" }))
      setAlert(() => true)
    }
    finally{
  ...
  return (
    <div className='border-2 border-black w-3/5 mx-auto pt-2 pb-2'>
      ...
      />
      <div className={`flex bg-green-100 rounded-lg p-4 mb-4 text-sm text-green-700 ${alert ? '' : 'hidden'}`} role="alert">
        ...
        <div>
          <span className="font-medium">{alertData.text}</span> {alertData.messageBody}
        </div>
      </div>
    </div>
  )
